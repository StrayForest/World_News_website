...
def country_news(request, country_name):
    ...
    news_list = News.objects.filter(country_name=country).exclude(content_review__isnull=True).exclude(
        content_review='').exclude(content_review__istartswith='Error').order_by('-date', '-searches')[:20]
